...
所有命令按执行位置分为三类，每条命令均标注归属：

| 环境 | 进入方式 | 镜像 | 用途 |
|---|---|---|---|
| **🐳 roscore_host 容器** | `docker exec -it roscore_host bash` | `simulator01` | ROS CLI（rostopic / rosservice / rosbag 等），已 source Noetic，`--net host` 直连 `localhost:11311`。工作目录不限。 |
| **🐳 viz_all 容器** | `docker exec -it viz_all bash` | `simulator01-viz` | GUI 工具（rviz / rqt_image_view），带 X11 转发 + 软件渲染。前置：`./scripts/run_viz.sh` 已启动。工作目录不限。 |
| **🖥️ 宿主机** | 普通终端 | — | Ubuntu 22.04 + ROS Humble。编辑配置文件、启动脚本、运行 UE 模拟器。**不能直接跑 Noetic ROS 命令。** |

> 三种环境的启动脚本均在 `simulator/scripts/` 下，需先在宿主机 `~/RMUA/simulator/` 目录执行：
> `./scripts/run_roscore.sh` → `./scripts/run_host_window.sh` → `./scripts/run_viz.sh`
...
> 🐳 执行位置：**roscore_host 容器**，进入方式：`docker exec -it roscore_host bash`

| 目的 | 命令 |
|---|---|
| 列全部话题 | `rostopic list` |
| 列全部服务 | `rosservice list` |
| 看话题类型 | `rostopic type /airsim_node/drone_1/imu/imu` |
| 看话题发布者/订阅者(**md5 不匹配时订阅者为 0**) | `rostopic info /airsim_node/drone_1/vel_body_cmd` |
| 看服务类型/参数 | `rosservice info /airsim_node/drone_1/takeoff` |
| 看话题频率 | `rostopic hz /airsim_node/drone_1/imu/imu` |
| 看一条数据 | `rostopic echo -n1 /airsim_node/drone_1/debug/pose_gt` |
| 录包 | `rosbag record -O /tmp/run.bag /airsim_node/drone_1/imu/imu /airsim_node/drone_1/vel_body_cmd` |
| 回放 | `rosbag play /tmp/run.bag` |
...
> 带 `*` 为推断，**以 `rostopic type <话题>` 输出为准**。
> 每个相机还另有 `/Scene/camera_info`（内参）和 `/Scene/mouse_click` 子话题。

常用：

> 🐳 roscore_host 容器（`docker exec -it roscore_host bash`）：
```bash
rostopic echo -n1 /airsim_node/drone_1/debug/pose_gt   # 当前位姿真值
rostopic hz   /airsim_node/drone_1/imu/imu              # IMU 频率(应≈100)
rostopic echo /airsim_node/drone_1/debug/rotor_pwm      # 四电机实时 PWM
```

> 🐳 viz_all 容器（需 GUI，`docker exec -it viz_all bash`，前置 `./scripts/run_viz.sh`）：
```bash
rqt_image_view                                           # 看相机画面
rviz -d /tmp/rmua_full.rviz                              # 雷达点云+相机（容器内路径）
```
...
rostopic pub -1 /airsim_node/drone_1/vel_body_cmd airsim_ros/VelCmd \
...
```bash
rostopic pub -1 /airsim_node/drone_1/rotor_pwm_cmd airsim_ros/RotorPWM \
...
```bash
# 起飞(已验证可调用)
rosservice call /airsim_node/drone_1/takeoff "{waitOnLastTask: true}"
# 降落
rosservice call /airsim_node/drone_1/land "{waitOnLastTask: true}"
# 重置当前局
rosservice call /airsim_node/reset
```
...
1. **ROS CLI 命令在 `roscore_host` 容器里跑**（`docker exec -it roscore_host bash` 进入），GUI 工具在 `viz_all` 容器里跑。宿主 Humble 与 Noetic 冲突，不要直接在宿主跑 ROS 命令。
...
5. **VEL/PWM 首发锁定**，别混用。
6. 🖥️ **宿主机**：开发前编辑 `~/RMUA/simulator/Build/LinuxNoEditor/RMUA/Content/Configs/GameConfig.json`，把 `IgnoreAllHitCollision`/`IgnoreOverTime` 设 `true` 并重启 UE，否则超时/撞击结束本局、无人机随后无视所有指令（reset 后能动、悬停十几秒后卡死即此现象）。
